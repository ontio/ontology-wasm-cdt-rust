...
pub trait Oep8Token {
    ...
    fn approve(&mut self, owner:&Address, spender:&Address,amount:U256, token_id:String) -> bool;
    fn allowance(&mut self, owner:&Address, spender:&Address, token_id:String) -> U256;
    fn transfer_from(&mut self, spender:&Address,from:&Address, to:&Address,amount:U256, token_id:String) -> bool;
...
impl Oep8Token for Oep8TokenInstance {
    ...
    fn transfer_multi(&mut self, states:&[(Address,Address,U256,String)]) -> bool {
        ...
        for state in states.iter() {
            if self.transfer(&state.0, &state.1, state.2, state.3.clone()) == false {
                panic!("transfer failed, from:{},to:{},amount:{},token_id:{}", state.0, state.1, state.2, state.3);
            }
        }
        true
    }
    ...
    fn approve_multi(&mut self, obj:&[(Address, Address, U256, String)]) -> bool {
        ...
        for o in obj.iter() {
            if self.approve(&o.0,&o.1,o.2,o.3.clone()) == false {
                panic!("approve_multi failed! from:{}, to:{}, amount: {},token_id:{}", &o.0,&o.1,o.2,o.3);
            }
        }
        true
    }
    fn transfer_from_multi(&mut self, obj:&[(Address, Address, Address, U256, String)]) -> bool {
        ...
        for o in obj.iter() {
            if self.transfer_from(&o.0,&o.1,&o.2,o.3, o.4.clone()) == false {
                panic!("transfer_from_multi failed, spender:{}, from:{}, to:{}, amount:{},token_id:{}",
                       &o.0,&o.1,&o.2,o.3, o.4);
            }
        }
        true
    ...
    //optional
    fn create_multi_type_token(&mut self) -> bool {
        let token_name_list = ["TokenNameFirst", "TokenNameSecond", "TokenNameThird", "TokenNameFourth", "TokenNameFifth"];
        let token_symbol_list = ["TNF", "TNS", "TNH", "TNO", "TNI"];
        let token_supply_list = [U256::from(100000), U256::from(200000), U256::from(300000), U256::from(400000), U256::from(500000)];
        for index in 0..5 {
            ...
            database::put(&utils::concat(token_id.clone(), (BALANCE, Address::zero())), token_total_supply);
            self.Transfer(&Address::zero(), &Address::zero(), token_total_supply, token_id);
        }
        true
    }
    fn check_token_id(&self, token_id:String) -> bool {
